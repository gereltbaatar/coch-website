...
// Image preview type - stores both file and preview URL
...
export default function AddProductPage() {
    ...
    const [price, setPrice] = useState("") // Stores formatted value like "10,000"
    const [discountPrice, setDiscountPrice] = useState("") // Stores formatted discount price
    const [selectedCategory, setSelectedCategory] = useState<string>("")
    const [buyType, setBuyType] = useState<ProductBuyType>("fb_messenger")
    const [buyLink, setBuyLink] = useState("")
    const [isSaving, setIsSaving] = useState(false)

    // Images state - stores File objects with local preview URLs
    ...
    const handleImageSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setImageFiles(prev => [...prev, ...newPreviews].slice(0, 5))
    ...
    const handleSave = async (status: ProductStatus) => {
        ...
        try {
            ...
            toast.success(status === 'active' ? "Бүтээгдэхүүн нэмэгдлээ!" : "Ноорог хадгалагдлаа!")
